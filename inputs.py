...
def empty_model_opts():
    opts = ["pairs_file","pairwise_params_file",
            ...
            "pairwise_type","verbose"]         
    modelopts = { opt:None for opt in opts }
    return modelopts

def add_pairwise_params(modelopts):
...
def load_config(name):
    ...
    modelopts = empty_model_opts()
    fittingopts = empty_fitting_opts()
...
def load_models(names,dry_run=False):
    ...
    for name in names:
        ...
        model = load_model(name,dry_run=dry_run)
        Models.append(model)
    return Models

def new_models(names,options):
    """ Create new models with inputted options."""
    Models = []
    for name in names:
        options["PDB"] = "%s.pdb" % name
